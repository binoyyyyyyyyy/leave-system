...
/**
 * Get admin display details for certification snapshot.
 *
 ...
 *
 * This stores the form snapshot for that application, but the source of truth
 * remains the leave_credits table.
 */
...
function admin_leave_request_action(PDO $pdo, int $adminId, array $input): array
{
    ...
    $approvalName = trim((string)($input['approval_name'] ?? ''));
    $approvalPosition = trim((string)($input['approval_position'] ?? ''));

    $disapprovalName = trim((string)($input['disapproval_name'] ?? ''));
    $disapprovalPosition = trim((string)($input['disapproval_position'] ?? ''));
        ...
        if ($action === 'update_action') {
            ...
            $stmt = $pdo->prepare("
                ...
                SET
                    ...
                    disapproval_name = :disapproval_name,
                    disapproval_position = :disapproval_position
                WHERE id = :id
            ");
            ...
            $stmt->execute([
                ...
                ':approval_name' => $approvalName ?: null,
                ':approval_position' => $approvalPosition ?: null,
                ':disapproval_name' => $disapprovalName ?: null,
                ':disapproval_position' => $disapprovalPosition ?: null,
                ':id' => $id,
            ]);
...
function admin_leave_requests_list(PDO $pdo): array
{
    $requestsStmt = $pdo->query("
        SELECT
            ...
            la.approval_name,
            la.approval_position,
            la.disapproval_name,
            la.disapproval_position,
            la.days_with_pay,
            la.days_without_pay,
            la.others_specify,
